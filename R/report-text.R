...
report_field_catalog <- function() {
  general <- list(
    list(key = "country", group = "report", label = "Country"),
...
#'
#' @param cache A [CacheConnection] (or `NULL`: only the report's own fields are filled).
...
#' @export
report_fields <- function(cache, project, date = Sys.Date(), lang = "en") {
  ...
  years <- tryCatch(cache$data_years, error = function(e) NULL)
  if (length(years)) {
...
#' @param cache A [CacheConnection] (for the latest year of data), or `NULL`.
#' @param i18n The report's translations (`NULL`: English).
#' @return The blocks, their texts filled.
#' @export
report_chart_fields <- function(blocks, cache = NULL, i18n = NULL) {
  if (!length(blocks)) return(blocks)
  years <- tryCatch(if (!is.null(cache)) cache$data_years, error = function(e) NULL)
  ...
  for (i in seq_along(blocks)) {
    ...
    names_ <- unique(unlist(lapply(group, function(g) if (is.character(g$indicator) && nzchar(g$indicator)) .rb_ind_name(i18n, g$indicator))))
    values <- list(
      chart_indicators = if (length(names_)) .rb_join_and(names_, i18n),
      chart_indicator = if (!is.null(target$indicator) && nzchar(target$indicator)) .rb_ind_name(i18n, target$indicator),
