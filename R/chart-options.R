...
#' `options` argument (and any of these fields as a named argument in `...`), and [CacheConnection] can keep them per
...
cd_chart_options <- function(..., title = NULL, subtitle = NULL, caption = NULL, tag = NULL,
  ...
  if (length(extra)) {
    ...
    if (length(bad)) {
      cd_abort(c("x" = "Unknown chart option: {.field {bad}}.",
...
.validate_chart_options <- function(opts) {
  ...
  if (length(bad)) cd_abort(c("x" = "Unknown chart option: {.field {bad}}."))
...
# Normalise what a caller may hand in -- NULL, a list of fields, or an object -- to a cd_chart_options
as_chart_options <- function(x) {
  ...
  cd_abort(c("x" = "{.arg options} must be NULL, a list, or a {.cls cd_chart_options} object."))
}
