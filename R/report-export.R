...
.report_chart_options <- function(cache, design = NULL) {
  saved <- cache$chart_options %||% list()
  ...
  list(default = merge_chart_options(base, cache$get_chart_options("default")), types = types)
}
...
#' @export
with_report_chart_options <- function(cache, code, design = NULL) {
  old <- options(cd2030.report_chart_options = .report_chart_options(cache, design))
...
.rb_word_finish <- function(docx, pdf = NULL) {
  ...
  if (!identical(as.integer(status), 0L) || (!is.null(pdf) && !file.exists(pdf))) {
    cd_abort(c("x" = "Microsoft Word could not make the PDF.", "i" = paste(utils::tail(out, 3), collapse = " ")))
  }
  invisible(TRUE)
}
...
.rb_libreoffice_pdf <- function(docx, pdf) {
  ...
  if (!file.exists(made)) cd_abort(c("x" = "LibreOffice could not make the PDF.", "i" = paste(utils::tail(out, 3), collapse = " ")))
...
#' @export
export_report <- function(cache, project, file, format = c("docx", "pdf", "pptx"), i18n = NULL, subtitle = NULL, progress = NULL,
  ...
  if (deck) {
    if (format == "docx") cd_abort(c("x" = "A slide deck is written as PowerPoint ({.val pptx}) or PDF, not Word."))
    converter <- switch(converter, word = "powerpoint", browser = "auto", converter)
    return(export_deck(cache, project, file, format, i18n = i18n, progress = progress, converter = converter))
  }
  if (format == "pptx") cd_abort(c("x" = "A document is written as Word ({.val docx}) or PDF; only slide decks are PowerPoint files."))
  ...
  parts <- .rb_prepare(cache, project, dir, i18n, function(x) step(0.7 * x))
...
#'   the caller removes.
#' @export
report_final_pages <- function(cache, project, i18n = NULL, dpi = 60, progress = NULL) {
  deck <- identical(project$kind, "deck")
  converter <- if (deck) report_deck_converter() else report_converter()
  if (is.null(converter)) {
    cd_abort(c("x" = if (deck) "Showing the final slides needs Microsoft PowerPoint or LibreOffice on the computer running the app."
               else "Showing the final pages needs Microsoft Word or LibreOffice on the computer running the app."))
  }
  if (!requireNamespace("pdftools", quietly = TRUE)) cd_abort(c("x" = "Showing the final pages needs the {.pkg pdftools} package."))
  out <- tempfile("pages_")
  dir.create(out)
  pdf <- file.path(out, "report.pdf")
  export_report(cache, project, pdf, "pdf", i18n = i18n, progress = function(x) if (is.function(progress)) progress(0.9 * x), converter = converter)
...
.rb_prepare <- function(cache, project, dir, i18n, step) {
  design <- .rb_design(project$design)
  cover <- .rb_cover(project$cover)
  regions <- .rb_regions(cache)
  ...
  fields <- report_fields(cache, project, lang = (if (is.list(i18n)) i18n$lang) %||% "en")
  blocks <- report_chart_fields(blocks, cache, i18n)
  ...
  with_report_chart_options(cache, design = design, {
    for (k in seq_along(todo)) {
      i <- todo[[k]]
      b <- blocks[[i]]
      if (identical(b$type, "canvas")) {
        # a free-layout page: each of its items drawn at its box (R/report-canvas.R)
        rendered[[i]] <- .rb_canvas_prepare(cache, b, project, design, i18n, file.path(dir, paste0("canvas_", i)), vector)
        ...
        if (is.character(b$src) && startsWith(b$src, "asset:")) b$src_file <- .rb_asset_file(cache, b$src, file.path(dir, paste0("asset_", i)))
        rendered[[i]] <- .rb_image_file(b, file.path(dir, paste0("image_", i)), report_block_size(b, design))
      } else {
        r <- render_report_block(cache, b, i18n, design)
  ...
  cover_files <- list(
    flag = if (isTRUE(cover$show_flag)) .rb_flag_file(tryCatch(cache$country_iso, error = function(e) NULL)),
...
# A picture kept in the dataset (cache$set_report_asset()) written to a file; NULL when there is none
.rb_asset_file <- function(cache, src, path) {
  id <- sub("^asset:", "", src)
  asset <- tryCatch(cache$report_assets[[id]], error = function(e) NULL)
...
#' @param cache A [CacheConnection] object.
#' @param id The picture's id (with or without `"asset:"`).
#' @return A `data:` URL, or `NULL` when the dataset has no such picture.
#' @export
report_asset_data_url <- function(cache, id) {
  asset <- tryCatch(cache$report_assets[[sub("^asset:", "", id)]], error = function(e) NULL)
  if (is.null(asset) || !is.raw(asset$data)) return(NULL)
...
#' @export
report_store_asset <- function(cache, id, src) {
  if (!is.character(src) || length(src) != 1 || !nzchar(src)) cd_abort(c("x" = "{.arg src} must be a data URL or a web address."))
  file <- tempfile()
  ...
  if (startsWith(src, "data:")) {
    ...
    if (!identical(ok, 0L)) cd_abort(c("x" = "The picture could not be downloaded from {.url {src}}.", "i" = if (is.character(ok)) ok))
  } else {
    cd_abort(c("x" = "{.arg src} must be a data URL or a web address."))
  }
  if (!requireNamespace("magick", quietly = TRUE)) cd_abort(c("x" = "The {.pkg magick} package is needed to store pictures."))
  img <- tryCatch(magick::image_read(file)[1], error = function(e) NULL)
  if (is.null(img)) cd_abort(c("x" = "This is not a picture (PNG, JPEG, GIF, WebP or SVG)."))
  ...
  cache$set_report_asset(id, list(type = type, data = data))
...
#' A country's flag
#'
#' Downloaded once from flagcdn.com and kept in the package's cache folder. `NULL` when the country is not known or the
...
.rb_flag_file <- function(iso3) {
  ...
  dir <- tools::R_user_dir("cd2030.core", "cache")
...
.rb_write_docx <- function(parts, file, i18n) {
  ...
  template <- own$file %||% system.file("rmd", "report-template.docx", package = "cd2030.core")
...
.rb_write_pdf <- function(parts, file, dir, i18n) {
  if (!requireNamespace("chromote", quietly = TRUE)) {
    cd_abort(c("x" = "PDF export needs Microsoft Word, LibreOffice, or the {.pkg chromote} package with Chrome.", "i" = "Download as Word instead."))
  }
  .rb_find_browser()
  design <- parts$design
...
.rb_find_browser <- function() {
  ...
  hit <- candidates[nzchar(candidates) & file.exists(candidates)]
  if (!length(hit)) cd_abort(c("x" = "PDF export needs Microsoft Word, LibreOffice, Google Chrome, Microsoft Edge or Chromium.", "i" = "Download as Word instead."))
