...
.rb_powerpoint_pdf <- function(pptx, pdf) {
  ...
  if (!identical(as.integer(status), 0L) || !file.exists(pdf)) {
    cd_abort(c("x" = "Microsoft PowerPoint could not make the PDF.", "i" = paste(utils::tail(out, 3), collapse = " ")))
  }
  invisible(TRUE)
}
...
#' @export
export_deck <- function(cache, project, file, format = c("pptx", "pdf"), i18n = NULL, progress = NULL,
  ...
  if (format == "pdf" && converter == "auto") {
    converter <- report_deck_converter() %||%
      cd_abort(c("x" = "Making a PDF of slides needs Microsoft PowerPoint or LibreOffice.", "i" = "Download as PowerPoint instead."))
  }
  ...
  .rb_write_pptx(cache, project, pptx, dir, i18n, function(x) step(if (format == "pdf") 0.8 * x else x))
  if (format == "pptx") {
    step(1)
...
.rb_write_pptx <- function(cache, project, file, dir, i18n, step) {
  ...
  fields <- report_fields(cache, project, lang = project$lang %||% (if (is.list(i18n)) i18n$lang) %||% "en")
  ...
  with_report_chart_options(cache, design = design, {
    for (s in seq_along(slides)) {
      ...
      blocks <- .rb_deck_chart_fields(blocks, cache, i18n)
      for (k in seq_along(items)) {
        x <- .rb_deck_item(x, items[[k]], blocks[[k]], cache, design, fields, i18n, file.path(dir, paste0("s", s, "_", k)))
        done <- done + 1
        step(done / total)
      }
      notes <- slide$notes
...
.rb_deck_chart_fields <- function(blocks, cache, i18n) {
  ...
  for (i in which(!is_data)) {
    ...
    blocks[[i]] <- report_chart_fields(c(list(blocks[[i]]), charts), cache, i18n)[[1]]
  }
  blocks
}
...
.rb_deck_item <- function(x, item, b, cache, design, fields, i18n, path) {
  ...
  if (type == "image") {
    ...
    if (is.character(b$src) && startsWith(b$src, "asset:")) b$src_file <- .rb_asset_file(cache, b$src, paste0(path, "_asset"))
