...
.rb_canvas_prepare <- function(cache, canvas, project, design, i18n, path, vector = FALSE) {
  ...
  for (k in seq_along(items)) {
    ...
    if (type == "image") {
      ...
      if (is.character(b$src) && startsWith(b$src, "asset:")) b$src_file <- .rb_asset_file(cache, b$src, paste0(file, "_asset"))
