...
report_theme_from_file <- function(path, name = NULL) {
  ...
  if (is.null(kind)) cd_abort(c("x" = "This is not a PowerPoint or Word file (.pptx, .potx, .docx, .dotx)."))
  dir <- tempfile("theme_")
  on.exit(unlink(dir, recursive = TRUE), add = TRUE)
  utils::unzip(path, exdir = dir)

  out <- report_themes()$countdown
...
.rb_deck_template <- function(path, size = NULL) {
  ...
  path <- path[1]
  fail <- function(why) {
    cd_warn(c("!" = "The PowerPoint template could not be used; the deck is written without it.", "i" = why))
    NULL
  }
...
.rb_docx_template <- function(path) {
  ...
  path <- path[1]
  fail <- function(why) {
    cd_warn(c("!" = "The Word template could not be used; the document is written without it.", "i" = why))
    NULL
  }
  if (!file.exists(path)) return(fail("The template file was not found."))
  kind <- .rb_office_kind(path)
  ...
  result <- tryCatch({
    ...
    utils::unzip(system.file("rmd", "report-template.docx", package = "cd2030.core"), exdir = own)
