...
plot_subnational_comparison <- function(.data, indicator = c('penta3', 'ideliv'), denominator = c('penta1', 'anc1', 'dhis2', 'penta1derived'), last_year) {
  check_required(.data)
...
```{r, fig.width=3.6, fig.height=1.5}
admin1_cov %>% 
  filter_coverage('ideliv', denominator = cache$get_denominator('ideliv'), region = adminlevel_1) %>% 
  plot() +
  cd_report_theme(base_size = 7) +
  theme(
    legend.position = 'none',
...
```
```{r, fig.width=3.6, fig.height=2.5}
plot_subnational_comparison(admin1_cov, 'ideliv', denominator = cache$get_denominator('ideliv'), last_year = last_year)
```
